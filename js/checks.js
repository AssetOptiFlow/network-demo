...
import { components } from "./roads.js";
...
export function runChecks(world) {
  ...
  // 5. roads cross the river only at generated bridges
  let badCrossings = 0;
  let badInfo = "";
  ...
  add("Roads cross river only at bridges", badCrossings === 0,
    `${badCrossings} non-bridge river crossing edge(s), ${world.terrain.bridges.length} bridge(s)` + badInfo);
...
export function checkFaultConservation(scenario) {
  ...
  return {
    ...
    detail: `${scenario.custOut} out + ${scenario.custIso} isolatable = ` +
      `${scenario.custOut + scenario.custIso} of ${scenario.custAffected} affected` +
